...
const TechStack = ({ techs }) => {
    ...
    return (
        <div className="grid w-full grid-cols-4 gap-y-10">
            {techs.map((t) => (
                <div className="group relative bg-gray-50 border border-gray-100 rounded-full flex items-center justify-center w-min p-5 cursor-pointer">
                    <ReactSVG key={t} src={process.env.PUBLIC_URL + "/images/icons/" + t.toLowerCase() + ".svg"} beforeInjection={svgStyle} />
                    <span className="absolute top-full mt-2 text-sm font-inter opacity-0 group-hover:opacity-100">{t}</span>
                </div>
            ))}
        </div>
    );
